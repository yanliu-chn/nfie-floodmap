...
def updateMax(ncdir = '', ncfile = '', comidlist = [], Qlist = [], h = {}):
    f = ncdir + "/" + ncfile 
...
def calcMax(ncdir = '', nclist = None, comidlist = [], Qlist = []):
    ...
    for ncfile in nclist:
        ...
        sys.stdout.flush()
        updateMax(ncdir, ncfile, comidlist, Qlist, h)
    h = None

# save as netcdf
def saveWorstFC(of = '', comidlist = [], Qlist = [], samplefile = ''):
...
if __name__ == '__main__':
    ...
    comidlist = []
    Qlist = []
    calcMax(ncdir, nclist, comidlist, Qlist)
    saveWorstFC(of, comidlist, Qlist, ncdir + '/' + nclist[len(nclist)-1]) # use last nc timestamp as fc timestamp

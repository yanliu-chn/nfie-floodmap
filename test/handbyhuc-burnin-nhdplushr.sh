...
Tstart
	dsnhdplushr=/gpfs_scratch/usgs/nhd/nhdplus-hr-beta/NHDPlus_H_1209_GDB.gdb
    geolayer=NHDPlusBurnLineEvent
    vaalayer=NHDPlusFlowlineVAA 
    echo "python $sdir/nhdplushr-flowline.py $hucid $dsnhdplushr NHDPlusID . ${n}-flows" && \
    [ ! -f "${n}_flows_hr.shp" ] && \
    python $sdir/nhdplushr-flowline.py $hucid $dsnhdplushr NHDPlusID . ${n}_flows_hr && \
    [ $? -ne 0 ] && echo "ERROR creating flowline shp." && exit 1
Tcount burnflowline_nhdplushr

...
def forecastH (init_timestr = None, timestr = None, tablelist = None, numHeights = 83, huclist = None, odir = None, nhddbpath = None):
    ...
    for i in range(0, len(tablelist)): # scan each HUC's hydro prop table
        ...
        catchcount += (colcatchid.size ) # 2D data
        ...
        for i in range(colcatchid.size):
            ...
            if not catchid in h: # hydro table doesn't have info for this comid
                ...
                continue
            count = 0
            for j in range(colH.size):
                Hlist[count] = colH[j]
                Qlist[count] = colQ[i][j]
                count += 1
            comid = catchid
            ...
            Hlist.fill(0)
            Qlist.fill(0)
    print datetime.now().strftime("%Y-%m-%d %H:%M:%S : ") + "Read " + str(len(comids)) + " stations from NWM, " + str(catchcount) + " catchments from hydro table. " + str(missings) + " comids in hydro table but not in NWM. " + str(nulls) + " comids null and skipped. " + str(fccount) + " forecasts generated."
    sys.stdout.flush()
...
if __name__ == '__main__':
    hpinput = sys.argv[1] # hydro property file/dir, 2D index [CatchId, Stage]
